...
## 8. Resolved: preorder_weighted_impl handles undirected input

**Verified** by reading TreeTools source (`renumber_tree.h`, lines 262-268).
The function builds a `parent_of[]` map from the input edges
(`parent_of[child[i]] = parent[i]` for each edge), then finds the root
as the node with `parent_of == 0` (i.e., nTip+1). Children are re-sorted
by smallest descendant. Because it re-roots from the known root node,
**arbitrary/inconsistent edge directions are handled correctly** — it
treats the input as an undirected graph.

This confirms that our undirected-manipulation approach (Section 5) is
safe: after performing suppress + regraft on both sides, even if v
temporarily has two parent edges (bisectRow and regRowB),
`preorder_weighted_impl` will produce a correctly rooted, directed,
preorder-canonical result.
...
### 10a. C++ function signature

In `src/tree_moves.cpp`, add:

```cpp
// [[Rcpp::export]]
List tbr_proposal(IntegerMatrix edge, int nTip,
                  double treeLength, NumericVector relBrLengths);

// Internal impl (called by mcmc.cpp dispatch)
List tbr_proposal_impl(IntegerVector parent, IntegerVector child,
                       int nTip, double treeLength,
                       NumericVector relBrLengths);
```

Follow the same pattern as `nni_proposal_impl` / `spr_proposal_impl`:
- `_impl` accepts vectors directly (no matrix conversion)
- Exported wrapper converts edge matrix ↔ vectors
- Return `List::create(_["parent"], _["child"], _["rel_br_lengths"], _["logHastings"])`

### 10b. MCMC dispatch (src/mcmc.cpp)

Add forward declaration:

```cpp
List tbr_proposal_impl(IntegerVector, IntegerVector, int, double, NumericVector);
```
...
### 10c. R-side registration (R/RunMkPrime.R)
...
Add to `.BuildMoves()` inside the `if (!fixTopology && nEdge >= 5L)` block,
unconditionally (like NNI/SPR — the adaptive scheduler handles frequency):

```r
list(name = "tbr", type = "tbr", target = NULL,
     weight = max(1, nEdge / 6), dim = 1L)
```

Weight rationale: TBR is more expensive than SPR (two regrafts) but has
larger mixing potential. `nEdge / 6` gives it lower initial frequency
than SPR (`nEdge / 4`). The bandit will adjust.

### 10d. R wrapper (R/proposals.R)
...
ProposeTbr <- function(tree, tree_length, rel_br_lengths) {
  ...
  newTree <- tree
  newTree$edge <- cbind(result$parent, result$child)
...
### 10e. Implementation order

1. ~~Verify `preorder_weighted_impl` behavior~~ (done — §8).
2. C++ `tbr_proposal_impl` + `tbr_proposal` in `src/tree_moves.cpp`.
3. Unit tests for proposal validity (`test-tbr.R`).
