...
//           33=scale_hyper_tau (Bactrian scale on the population scale τ of
...
static bool do_move_impl(McmcData* data, McmcState* state,
  ...
  bool             case33Active   = false;
  double           oldTau33       = 0.0;
  NumericVector    oldClassRLS33;  // snapshot of classRateLogSd (all entries)
  double           oldRateLogSd33 = 0.0;
  ...
  switch (moveType) {
    ...
    case 33: { // scale_hyper_tau — Bactrian scale on the population scale τ.
      ...
      case33Active   = true;
      oldTau33       = state->hyperTau;
      oldClassRLS33  = clone(state->classRateLogSd);
      oldRateLogSd33 = state->rateLogSd;
      double mult = std::exp(scaleTuning * bactrian_perturbation());
      double newTau = oldTau33 * mult;
      ...
      for (int k = 0; k < state->classRateLogSd.size(); ++k) {
        ...
        if (state->classRateLogSd[k] <= 0.0) {
          ...
          state->hyperTau     = oldTau33;
          state->classRateLogSd = oldClassRLS33;
          state->rateLogSd    = oldRateLogSd33;
          return false;
        }
      }
  ...
  if (R_FINITE(logAlpha) && std::log(R::unif_rand()) < logAlpha) {
    ...
    if (!usedPartialCL && likChanges) {
      switch (moveType) {
        ...
        case 33:  // scale_hyper_tau: rescales all σ_c → ACRV rates change
  ...
  if (nniInPlace) { state->parent[nniCRow] = nniSavedP_cRow; state->parent[nniWRow] = nniSavedP_wRow; }
  // Per-class rollback (cases 31, 32, 33)
  if (classIdx31 >= 0) {
    state->classRateLogSd[classIdx31] = oldClassRLS;
    if (state->useHyperpriorOnSigma &&
        classIdx31 < (int)state->classZ.size()) {
