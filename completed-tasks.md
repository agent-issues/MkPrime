...
| M-093 | Gibbs update for p (conjugate Beta draw) | B | 2026-03-27 | Phase 9. `feature/gibbs-weighted-moves`. moveType 9 = `gibbs_p` in `do_move_impl()`. Full conditional p \| k' ~ Beta(a+nTrans, b+Σ(k'−kObs)). No likelihood recomputation. Always returns true. `fill_partition_cache()` now syncs `state->logLik` from C++ partition sum. Logseries guard: p move only added when `kPrimePrior != "logseries"`. 63 new gibbs tests; plot test fixed to use temp device. 335 MCMC engine tests pass. |
| M-083 | Extract `compute_full_loglik()` / `compute_full_loglik_at()` C++ helpers | B | 2026-03-27 | `feature/gibbs-weighted-moves`. Two static helpers in `mcmc.cpp`: `compute_full_loglik(data, state)` evaluates at current tree; `compute_full_loglik_at(data, state, parent, child, edgeLen)` evaluates at an arbitrary topology using state's kPrime/rates/workspace. Two Rcpp-exported wrappers for R-level testing. 12 tests (workspace vs. no-workspace, immutability, NNI candidate, determinism). Discovered postorder invariant requirement; documented in fixture. P1 blocker for M-085–M-089. |
| M-084 | Subtree swap topology operation + `get_valid_swap_partners()` | B | 2026-03-27 | `feature/gibbs-weighted-moves` `fc37ccd`. `swap_subtrees_impl` + `get_valid_swap_partners_impl` in `tree_moves.cpp`. Exchanges attachment points of two non-nested, non-sibling subtrees; branch lengths swap with subtrees (Jacobian=1); result reordered to postorder. Rcpp-exported wrappers. 33 tests covering round-trip, tip/internal swaps, tree-length preservation, branch-length-follows-subtree, partner exclusion, root rejection. |
| M-085 | GibbsSPR | B | 2026-03-27 | `feature/gibbs-weighted-moves`. `gibbs_spr_impl()` static function in `mcmc.cpp`, moveType 10. Enumerates all valid SPR reattachment edges, weights by exp(β×logLik) (prior topology-independent), includes current state in candidate set (proper discrete Gibbs), samples proportionally, applies chosen topology at tau=0.5. clears partLogLik cache on acceptance. 29 tests covering topology change, tree-length preservation, postorder invariant, scalar immutability, logLik sync, and statistical preference for better topologies. |
| M-086 | GibbsSubtreeSwap | B | 2026-03-27 | `feature/gibbs-weighted-moves`. `gibbs_subtree_swap_impl()` static function in `mcmc.cpp`, moveType 11. Same Gibbs pattern as M-085 but using `swap_subtrees_impl` (M-084). Enumerates valid swap partners via `get_valid_swap_partners_impl`, weights by exp(β×logLik), samples proportionally, applies chosen swap. clears partLogLik cache on acceptance. Tests share test file with M-085 (test-gibbs-spr.R). |
| M-093 | Gibbs update for p (conjugate Beta draw) | B | 2026-03-27 | Phase 9. `feature/gibbs-weighted-moves`. moveType 9 = `gibbs_p` in `do_move_impl()`. Full conditional p \| k' ~ Beta(a+nTrans, b+Σ(k'−kObs)). No likelihood recomputation. Always returns true. `fill_partition_cache()` now syncs `state->logLik` from C++ partition sum. Logseries guard: p move only added when `kPrimePrior != "logseries"`. 63 new gibbs tests; plot test fixed to use temp device. 335 MCMC engine tests pass. |
