...
| M-071 | Progress bar redesign; nIter=Inf default | C | 2026-03-27 | `e3919a7`. `iter N \| logP \| ESS \| PSRF` format (no bar). `nIter=Inf` default; warmup=5000L fallback. Fixed latent `.CheckConvergence` bug: `converged=TRUE` when no criteria → now `FALSE` by default. |
...
| M-071 | Progress bar redesign + nIter=Inf default | C | 2026-03-27 | `f3ada92`. MkPrimeMCMC nIter=Inf default (warmup=5000L); for→repeat loop in RunMkPrime+ResumeMkPrime; new format "iter N \| logP: N \| ESS: N \| PSRF: N"; dynamic buffer doubling; convergence check added to ResumeMkPrime (was missing). 3211 tests pass. |
...
| M-076 | `MkpWatchLog()` — live trace plot from disk log files | C | 2026-03-27 | (see commit). Polling watcher + `.WatchLogPlot()` internal; `MkLogPaths()` path expander; 10 tests. |
