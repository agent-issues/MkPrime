...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| S-RED | dyn | OPEN | **Standing: Red-team review.** Review recent code changes for correctness bugs, edge cases, and safety issues. Focus areas: C++ MCMC hot path (likelihood, proposals, flat-buffer pruning, OPP-1–6 changes), MCMC acceptance logic (rollback correctness for each move type), R orchestration (convergence checking, streaming buffers, checkpoint/resume). File any bugs found as `u.nnn` issue files. When completed, record the focus area and outcome in the Notes column and reset to OPEN. Priority: ≥6 open tasks → P3, 3–5 → P2, <3 → P1. \| Last run: 2026-03-30 round 10 (B). Focus: areas touching Agent A's wall investigation — chain swap logic, heated acceptance arithmetic, tempering adaptation, kPrime proposals, slice sampler, pSPR Hastings ratio, adaptive tuning. **1 bug found: M-145.** Slice sampler (`slice_scalar_impl`) missing `nodeCL.valid = false` after acceptance. Affected: `slice_rate_log_sd` (all CLs stale), `slice_rate_loss`/`slice_rate_neo` (neomorphic CLs stale), `slice_beta_scale` (Q-het CLs stale). Same bug class as M-143. Plausible wall contributor: stale CLs after rate_log_sd slice → systematic NNI acceptance errors → wrong topology basin. Also reviewed without issues: chain swap Metropolis criterion, `std::swap(*states[i], *states[j])` correctness (all McmcState members are move-safe), heated acceptance (`beta * ΔlogLik + ΔlogPrior + logHR`), temperature adaptation (geometric ladder, bounds, overall swap rate), kPrime bounded integer walk (symmetric, correct boundary rejection), adaptive tuning floors (0.01 scale, 1L int_walk, 2.0 beta_simplex), pSPR Hastings ratio (forward/reverse candidate set asymmetry, branch-length Jacobian, parsimony weight ratio). |
