...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| M-118 | P1 | ASSIGNED (D) | **Bactrian proposal kernels for all scalar moves.** Replace `U - 0.5` uniform perturbation in scale proposals with a bimodal Bactrian kernel (Yang & Rodríguez 2013, m ≈ 0.95). Applies to: `tree_length`, `rate_loss`, `rate_neo`, `rate_log_sd`, `beta_scale`, concentration parameters. ~10 lines of C++ (`bactrian()` helper + swap into existing scale proposals). Expected gain: ~15–50% ESS/iter for scalar parameters, zero computational overhead. Now standard in BEAST2 ORC. See `research-mcmc-optimizations.md` §1. |
| M-119 | P2 | OPEN | **Parsimony-guided SPR (pSPR) proposal.** Weight SPR regraft candidates by `exp(-β × parsimony_score)` instead of uniform (standard SPR) or full likelihood (Gibbs SPR). Fitch downpass is O(n_char × n_tips) per candidate — essentially free for Mk data. Use as complement to Gibbs SPR during warmup when cheap exploratory moves are wanted. Correct Hastings ratio for asymmetric proposal. Note: `../TreeSearch` has highly optimized C++ parsimony routines (bitwise Fitch) that may be reusable via `LinkingTo`. Ronquist et al. 2020 report up to 10× convergence speedup. See `research-mcmc-optimizations.md` §2. |
