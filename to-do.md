...
|----|----------|--------|-------------|
| M-055 | P1 | OPEN | **Eliminate redundant `ape::reorder.phylo` in `MkpLogLikelihood()`.** The tree is *always* in postorder (invariant maintained by `.InitState()` and all topology proposals). The `ape::reorder.phylo(tree, "postorder")` call at likelihood.R:63 is pure waste — called on every single likelihood evaluation. Remove it; add a documented invariant comment. Also remove the `inherits()` and `match.arg()` validation from the hot-path code by creating an internal `.MkpLogLikelihood()` that skips checks, called from `.DoMove()`; the exported `MkpLogLikelihood()` retains validation for user-facing use. |
| M-056 | P1 | OPEN | **Pre-compute move weight vector.** `vapply(moves, [[, numeric(1), "weight")` is recomputed inside the innermost loop (per-chain, per-run, per-iteration) at RunMkPrime.R:135. Weights are constant except during adaptation (every 200 iterations). Compute once before the main loop and update only inside the adaptation block. |
