...
| M-093 | P2 | ASSIGNED (E) | **`MkBayesianServer` — PID liveness check on Reconnect.** When the user clicks Reconnect and no signal files are present (`mkp_done.signal`, `mkp_error.txt`, `mkp_cancel.signal`), the module currently defaults to `status = "running"` and polls indefinitely — even if the detached process was killed by the OS (OOM, shutdown) and will never write a signal. Fix: add `.PidIsAlive(pid)` internal helper (use `ps::ps_handle()`/`ps::ps_is_running()` if `ps` is installed; Windows fallback via `tasklist /FI "PID eq N"`; POSIX fallback via `tools::pskill(pid, 0L)`). In the Reconnect handler, when no signal files exist, call `.PidIsAlive(job$pid)`; if FALSE, set `rv$status <- "error"` with a clear notification ("process PID N is no longer running — may have been killed by the OS; partial log data available in logDir"). Add `ps` to Suggests. *(From u.398; S-RED M-078/M-079.)* Should be batched with M-082 (both touch the Reconnect handler). |
| M-082 | P2 | ASSIGNED (E) | **Checkpoint integration in EasyMkPrime (detached-process design).** In the new detached-process architecture (M-078/M-079), the MCMC script launched by `processx::process$new()` should write its checkpoint file into the same `logDir` as the TSV log files — no user file-system interaction needed. The job.rds (written by M-078) records the checkpoint path alongside `logFiles` and `cancelFile`, so the "Reconnect" button can resume a crashed/stopped run transparently. Specific tasks: (a) pass `checkpointFile = file.path(logDir, "checkpoint.rds")` when constructing the detached script; (b) "Reconnect" loads job.rds and checks for checkpoint existence before calling `RunMkPrime(overwrite=FALSE)`; (c) "Stop" flush + checkpoint are already guaranteed by M-081's cancel-file mechanism. If the checkpoint approach is not user-transparent, present the user with options (keep checkpoint vs. discard). *(From u.397.)* |
