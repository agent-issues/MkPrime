...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| M-156 | P1 | DONE | **Fast `exp()` approximation in pruning hot paths.** `std::exp()` accounts for 11.7% of CPU (VTune S-PROF4). Replace with a minimax polynomial or Padé approximation accurate to ~1e-8 relative error on [-25, 0] in all pruning paths (`pruning_jc_acrv_flat`, `pruning_mkn_acrv_flat`, `constant_site_prob_jc`, etc.). Standalone `fast_exp.h` header, toggled by compile-time flag for validation. Expected savings: 5–8% wall time. |
| M-157 | P1 | OPEN | **Fused ascertainment correction.** `constant_site_prob_jc` (6.6% CPU) runs a separate tree traversal with identical `exp()` arguments as the main pruning pass. Fold constant-site probability accumulation into the flat pruning functions (`pruning_jc_acrv_flat`, `pruning_mkn_acrv_flat`) to eliminate redundant traversals. Saves nCat × nEdge exp() calls per evaluation. Also consider fusing singleton-site correction for `coding = "informative"`. **Do not modify the `_persite` Gibbs paths** (Agent D territory). |
