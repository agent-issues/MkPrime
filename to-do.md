...
|----|----------|--------|-------------|
| M-178 | P3 | OPEN | **ESS plot x-axis uses scientific notation.** In `PlotDuringMCMC()`, the ESS-preview plot x-axis labels iterations as `1e+05` instead of `100k` (or `1.1M` etc.). Format with a human-readable suffix (k/M). File: `R/u.784` origin. |
| M-179 | P2 | OPEN | **PlotDuringMCMC: add tree-ESS and topology panel.** (a) Include tree-ESS estimates in the ESS panel whenever they are available. (b) Add a panel showing an approximate tree topology: MR consensus of ~12 recent posterior trees, optionally pruned with `QuickRogue`. Use `mar=rep(0,4)`, `cex≈0.7` to make good use of space. File: `u.893`. |
...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| M-176 | P2 | OPEN | **Checkpoint resume after reinstall loses log data.** After reinstalling MkPrime (R session restart), a resumed run shows `ℹ Resuming from checkpoint` but the log file is empty and samples from the prior run appear lost. Root cause unclear: the working directory at resume time may differ from the original run (reinstall triggers a session restart, resetting `getwd()`), so the log file path in the checkpoint no longer resolves correctly. Investigate whether checkpoint stores absolute or relative log paths, and whether `setwd` / project root changes silently redirect log output. File: `u.618`. |
...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| M-180 | P3 | OPEN | **Review minESS/s window and negative-ESS/s during warmup.** The min-ESS/s rate is used to estimate convergence ETA. The effective window for this estimate is unclear; during warmup the chain may move to a better optimum, causing apparent negative ESS/s and confusing the user. Clarify and document the window; consider suppressing or clamping the displayed value to ≥0 while warmup is ongoing. File: `u.820`. |
...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| M-181 | P3 | OPEN | **Refactor MkPrimeMCMC `moves` parameter.** `MkPrimeMCMC()` has grown many individual move toggle arguments. Coalesce into a single `moves` parameter accepting a named list (e.g. `list(nni = TRUE, tbr = FALSE)`) with per-move defaults. Retain backwards compatibility or emit a deprecation warning for old-style arguments. File: `u.826`. |
| M-182 | P3 | OPEN | **`ResumeMk(posterior)`: resume from a saved posterior object.** Allow users to continue sampling from an `MkPosterior` object recovered from disk (e.g. via `readRDS`) without needing the original `MkPrimeMCMC` control object. Determine what state must be embedded in `MkPosterior` to make this feasible. File: `u.015`. |
...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| M-183 | P3 | OPEN | **`@family` tags and `@seealso` for `RunMkPrime`.** Add `#' @family` tags to all exported functions so `has_concept` can drive `_pkgdown.yml` grouping. At minimum, add `@seealso` to `?RunMkPrime` pointing users to downstream functions (plotting, summarising, exporting posterior trees). File: `u.481`. |
| M-184 | P3 | OPEN | **Guide: reading log/trees files while MCMC runs in another session.** Add vignette or `?RunMkPrime` documentation section showing how to load partial results from disk (via `ReadMkLog()` / trees file) without interrupting an in-progress run, so users can begin building analytical workflows before convergence. File: `u.641`. |
| M-185 | P3 | OPEN | **hyoliths.qmd §Saving and resuming: document log files on disk.** The §Saving and resuming section should describe which log files are written, their format, and how a user might load them in external software (Tracer etc.). File: `u.654`. |
| M-186 | P3 | OPEN | **`MkPrime-package` documentation page.** Create `R/MkPrime-package.R` with a `@description` overview of the package, a summary of key exported functions grouped by purpose, and `@seealso` links to vignettes. File: `u.815`. |
