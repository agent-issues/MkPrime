...
| M-152 | P2 | OPEN | **Tuning bandit: minimum weight floor for topology moves.** The ESS/sec bandit can starve topology moves, causing large between-run mixing discrepancies. Observed on hyoliths (54 taxa, 225 chars): run 1 allocated ~18% to topology (tbr 8.7%, gibbs_subtree_swap 4.8%) and achieved rate_loss ESS 373 at 500 samples; run 2 allocated only ~5.6% total topology weight and achieved rate_loss ESS 54 — a 7× difference. `rate_loss` couples to topology through the neomorphic likelihood, so starving topology moves indirectly starves rate_loss exploration. **Proposed fix:** enforce a minimum aggregate weight floor for topology moves (e.g. 10–15%) so the bandit can reallocate within the category but cannot shrink the category below the floor. Consider also whether tuning information should be pooled across runs (shared initial weights from run 1 → run 2) to reduce variance. Relevant code: `.RunMkPrimeSingleRun()` tuning phase (line ~937), move weight perturbation logic. |
