...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| S-RED | dyn | OPEN | **Standing: Red-team review.** Review recent code changes for correctness bugs, edge cases, and safety issues. Focus areas: C++ MCMC hot path (likelihood, proposals, flat-buffer pruning, OPP-1–6 changes), MCMC acceptance logic (rollback correctness for each move type), R orchestration (convergence checking, streaming buffers, checkpoint/resume). File any bugs found as `u.nnn` issue files. When completed, record the focus area and outcome in the Notes column and reset to OPEN. Priority: ≥6 open tasks → P3, 3–5 → P2, <3 → P1. \| Last run: 2026-03-30 round 8 (B). Focus: Gibbs/weighted move acceptance paths and node CL cache lifecycle. **1 bug found → M-143.** Nine Gibbs/weighted acceptance paths (`gibbs_spr` ×3, `gibbs_subtree_swap` ×3, `weighted_spr`, `weighted_subtree_swap`, `block_gibbs_branch_sweep`) returned early from `do_move_impl` without invalidating `state->nodeCL.valid`. After accepted Gibbs topology moves, subsequent NNI/beta_simplex/Dirichlet moves would use stale partial CLs, computing incorrect log-likelihoods. Also reviewed: `gibbs_partial_cl.h` (evaluate_candidate, evaluate_swap_impl, ascertainment pseudo-groups), `ascertainment.cpp`, `tree_moves.cpp` (NNI, SPR, TBR, swap). Verified: flat Dirichlet(1,...,1) prior makes prior omission from Gibbs weights correct; `from_v` buffer reuse in partial CL lambda safe; BFS descendant computation bounds safe; TBR Hastings ratio (two Jacobian terms) correct. |
