...
| M-140 | P3 | OPEN | **Colour-code frozen move weights display.** The `"Move weights frozen"` message is a wall of text. Use `cli` colour: silver for param names and `=`, brighter colours for higher-weight moves, duller for rarer. *(from u.153)* |
| M-141 | P3 | OPEN | **ETA estimate during sampling.** After first convergence check, display estimated samples remaining and wall-clock ETA based on `minESS/s`. Prefer conservative (over-)estimate. *(from u.541)* |
TODOEOF 2>&1
