...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| M-169 | P2 | OPEN | **`het_constant_site_prob` per-edge-product bug in Gibbs kPrime sweep.** `het_constant_site_prob()` (mcmc_likelihood.cpp:1348) computes P(constant site) with `π_s × Π_e P_ss(t_e)` — a per-edge product that ignores tree topology (`parent`/`child` args unused). Correct formula requires Felsenstein pruning of k pseudo-characters. M-157 fixed the main hot-path (fused ascertainment), but `const_site_prob_for_k()` (line 1522, called from Gibbs kPrime sweep at mcmc.cpp:3218) still uses the buggy standalone. Affects k' inference when Q-het + variable coding + transformational chars are active. Fix: rewrite `het_constant_site_prob` to use F81 Felsenstein pruning over the tree (same approach as the M-157 fused pseudo-chars, but standalone). Filed from S-RED round 22 (u.170). |
...
| S-COORD | dyn | OPEN | **Standing: Coordination review.** Review to-do.md and completed-tasks.md for consistency. Check agent logs and u.nnn files. When completed, record round number and actions in Notes and reset to OPEN. Priority: same dynamic rule as S-RED. \| Last run: 2026-03-31 round 10 (B). Actions: (1) Killed 1 orphan Rscript (PID 21172, 142MB). (2) No u.* files. No remote jobs. (3) Agent D M-168 WIP in working tree: `gibbs_partial_cl.h` converts `f81Pi` from fixed `double[16]` to `std::vector<double>` (probable buffer overrun fix); `mcmc.cpp` 2 blank lines. (4) Task counts unchanged: 4 OPEN specific (M-158, M-161, M-166, M-131) + 2 ASSIGNED (M-168 D, M-080 C). Standing tasks at P2. Prev: round 9 (B). |
