...
| ID | Priority | Status | Description |
|----|----------|--------|-------------|
| M-112 | P2 | OPEN | **Cache CLGroup across consecutive Gibbs invocations.** After M-111, CLGroup setup (caching_downpass) is the dominant per-invocation cost for both Gibbs SPR and Gibbs swap — it's a full O(N×C) downpass recomputed from scratch each time. Within a single MCMC iteration, if the tree/parameters haven't changed between Gibbs SPR and Gibbs swap calls, the CLGroup is identical and the second downpass is wasted. Cache the CLGroup in the MCMC state and invalidate when topology or model parameters change. Expected saving: ~one O(N×C) downpass per iteration when both Gibbs moves are active. |
| M-113 | P2 | OPEN | **End-to-end mixing benchmark on production datasets.** Systematic comparison of ESS/s across move configurations (baseline SPR-only, +Gibbs, +Gibbs+partial CL stack) on Sun2018 (54 tips) and at least one larger dataset (100+ tips). M-091 benchmarked in debug build on a 23-taxon dataset — need production-build (-O2) results on realistic problem sizes where Gibbs moves are expected to shine (large trees with near-zero standard SPR acceptance). Report ESS/s for log_posterior, tree_length, and topology (RF-distance ESS via treess if available). |
