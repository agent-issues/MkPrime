...
# Self-cleaning: once the summary RDS is written and non-trivial in size,
# remove the raw stream + checkpoint files for this arm/task. Without this,
# 12 arms x 260 tasks accumulates ~400 GB and exhausts the /nobackup quota
# (see 2026-05-16 and 2026-05-20 incidents). Raws can be regenerated by
# re-running run_one.R for the specific arm/task if needed.
if (file.exists(out_file) && file.info(out_file)$size > 1024L) {
  raws <- c(log_file,
            tree_file,
            file.path(task_dir, sprintf("%s_run_2.log",    arm)),
            file.path(task_dir, sprintf("%s_checkpoint.rds", arm)))
  existing <- raws[file.exists(raws)]
  if (length(existing) > 0L) {
    unlink(existing)
    cat(sprintf("  Cleaned %d raw file(s) for %s/%s\n",
                length(existing), tag, arm))
  }
}
cat("Done.\n")
