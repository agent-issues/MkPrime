...
# remove the bulky raw stream files (log + nwk). KEEP the checkpoint so the
# run can be extended later with a longer walltime if needed
# (see feedback_resumable_runs.md). Without this cleanup, 12 arms x 260
# tasks accumulates ~400 GB and exhausts the /nobackup quota (see
...
if (file.exists(out_file) && file.info(out_file)$size > 1024L) {
  ...
  if (length(existing) > 0L) {
    unlink(existing)
    cat(sprintf("  Cleaned %d raw file(s) for %s/%s (checkpoint kept)\n",
