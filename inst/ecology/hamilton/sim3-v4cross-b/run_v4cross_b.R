# run_v4cross_b.R — Sim 3 v4-cross (balanced regime) blind+aware single-rep test.
#
...
#
# Uses SIM3V4_PARAMS$v4b (balanced regime: stemBrEco=0.08, stemBrClade=0.15).
...
outRoot <- file.path("/nobackup", Sys.getenv("USER"),
                     "mkp-sim3-v4cross-b", "results")
dir.create(outRoot, showWarnings = FALSE, recursive = TRUE)
cat(sprintf("=== Sim3 v4cross-b balanced test: rep%d seed=%d nIter=%d ===\n",
...
source(file.path(mkpRoot, "inst/ecology/simulations/sim3v4-helpers.R"))      # .BuildV4Data
source(file.path(mkpRoot, "inst/ecology/simulations/sim3v4cross-helpers.R")) # new
...
cat(sprintf("\n=== v4cross-b summary (rep %d, seed %d) ===\n",
