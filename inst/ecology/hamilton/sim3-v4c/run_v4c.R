# run_v4c.R — Sim 3 v4 (strong "stress" regime) blind+aware single-rep test.
#
# v4c: long eco stem (stemBrEco=0.15, near-saturation), short clade stems
# (stemBrClade=0.05).  Eco-shared signal expected to outweigh within-clade
# ancestry signal.  Target: blind chain actively supports the spurious
# {A1,A2,C1,C2} bipartition (P(eco12) > 0); aware chain still recovers true
# clade A and C monophyly and AC sister relationship.
#
# Signal ratio (per 250 chars):
#   v4c clade-stem ancestry ≈ 250 × P(change | r=1, t=0.05) ≈ 12 synapomorphies
#   v4c eco-shared          ≈ ~10+ parallel changes on both eco stems (t=0.15)
#   → ratio flips vs v4b (32 vs 5-8), giving the blind chain a real basis to
#   prefer the spurious clade.
#
...
outRoot <- file.path("/nobackup", Sys.getenv("USER"), "mkp-sim3-v4c", "results")
dir.create(outRoot, showWarnings = FALSE, recursive = TRUE)
cat(sprintf("=== Sim3 v4c stress test: rep%d seed=%d nIter=%d ===\n",
...
cat("\nTarget (v4c stress): blind P(eco12) > 0; aware recovers true clades.\n")
