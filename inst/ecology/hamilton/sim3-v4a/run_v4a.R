# run_v4a.R — Sim 3 v4 (conservative regime) blind+aware single-rep smoke test.
#
# v4a: short eco stem (stemBrEco=0.05, weak convergent signal), long clade
# stems (stemBrClade=0.30, strong ancestry signal).  Expect both chains to
# recover the true topology — this verifies the v4 harness before testing the
# interesting regime.
#
...
outRoot <- file.path("/nobackup", Sys.getenv("USER"), "mkp-sim3-v4a", "results")
dir.create(outRoot, showWarnings = FALSE, recursive = TRUE)
cat(sprintf("=== Sim3 v4a smoke test: rep%d seed=%d nIter=%d ===\n",
...
cat("\nExpected (v4a conservative): both chains P(AC) high, P(eco12) near 0.\n")
