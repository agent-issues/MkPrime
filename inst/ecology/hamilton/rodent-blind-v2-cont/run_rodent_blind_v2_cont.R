# run_rodent_blind_v2_cont.R --------------------------------------------------
# Continuation of the rodent BLIND Mk' MCMC v2 pilot (200k iter, minESS=21).
# Extends to 1M total iterations (800k additional) to reach minESS >= 200.
#
# Strategy: patch checkpoint$mcmc$nIter to 1000000L so ResumeMkPrime runs
# the remaining ~800k iterations beyond where the pilot stopped.
# The checkpoint file must exist at:
#   /nobackup/pjjg18/mkp-rodent-blind-v2/rodent-blind-v2.ckp
#
# Data rebuilt identically to run_rodent_blind_v2.R so character indices
# and taxon order align with the checkpoint state.
#
# Usage (via SBATCH script):
#   Rscript run_rodent_blind_v2_cont.R <nexFile>
#
suppressPackageStartupMessages({
  libPath <- Sys.getenv("MKP_LIB",
    file.path("/nobackup", Sys.getenv("USER"),
              "mkp-sim3-multirep-v3/lib"))
  .libPaths(c(libPath, .libPaths()))
  library("MkPrime")
  library("TreeTools")
})

args    <- commandArgs(trailingOnly = TRUE)
nexFile <- if (length(args) >= 1) args[1] else
             "~/downloads/mbank_X24848_2026-5-9-1135.nex"

stopifnot(file.exists(nexFile))

outRoot <- file.path("/nobackup", Sys.getenv("USER"), "mkp-rodent-blind-v2")
ckpFile <- file.path(outRoot, "rodent-blind-v2.ckp")
logFile <- file.path(outRoot, "rodent-blind-v2.log")

if (!file.exists(ckpFile)) {
  stop("Checkpoint not found: ", ckpFile,
       "\nRun run_rodent_blind_v2.R first.")
}

nIterTotal <- 1000000L  # 200k pilot + 800k continuation = 1M total

cat("=== Rodent BLIND v2 CONTINUATION ===\n")
cat("nexFile:   ", nexFile, "\n")
cat("ckpFile:   ", ckpFile, "\n")
cat("nIterTotal:", nIterTotal, "\n")
cat("outRoot:   ", outRoot, "\n")

# ---------------------------------------------------------------------------
# Patch checkpoint: extend nIter to 1M so ResumeMkPrime runs the remainder.
# ResumeMkPrime reads mcmc$nIter from the checkpoint; we update it in-place.
# ---------------------------------------------------------------------------
cat("Patching checkpoint nIter to", nIterTotal, "...\n")
ckp <- readRDS(ckpFile)
oldNIter <- ckp$mcmc$nIter
cat("  Previous nIter:", oldNIter, "-> new nIter:", nIterTotal, "\n")
ckp$mcmc$nIter     <- nIterTotal
ckp$mcmc$thin      <- max(1L, nIterTotal %/% 1000L)
ckp$mcmc$treeThin  <- max(1L, nIterTotal %/% 1000L)
saveRDS(ckp, ckpFile)
cat("Checkpoint patched.\n")

# ---------------------------------------------------------------------------
# Rebuild MkPrimeData — identical to run_rodent_blind_v2.R so indices align.
# ---------------------------------------------------------------------------
mat <- TreeTools::ReadCharacters(nexFile)
cat("Raw matrix:", nrow(mat), "tips x", ncol(mat), "chars\n")

ecologyCol <- 220L
extantCol  <- 221L
ecoVec  <- mat[, ecologyCol]
extant  <- mat[, extantCol]
keepTaxa <- which(extant == "0")  # 0 = extant, 1 = extinct
cat("Extant taxa:", length(keepTaxa), "of", nrow(mat), "\n")

matKeep <- mat[keepTaxa, -extantCol, drop = FALSE]
ecoKeep <- ecoVec[keepTaxa]
poly <- grepl("^\\(", ecoKeep)
if (any(poly)) {
  recoded <- substr(sub("^\\(", "", ecoKeep[poly]), 1, 1)
  ecoKeep[poly] <- recoded
  matKeep[poly, ecologyCol] <- recoded
}
hasEco <- !is.na(ecoKeep) & ecoKeep != "?" &
          !is.na(suppressWarnings(as.integer(ecoKeep)))
if (any(!hasEco)) {
  matKeep <- matKeep[hasEco, , drop = FALSE]
  ecoKeep <- ecoKeep[hasEco]
}

pdForDetect <- MatrixToPhyDat(matKeep[, -ecologyCol, drop = FALSE])
neoIdx <- AutoDetectNeomorphic(pdForDetect)
cat("Neomorphic chars:", length(neoIdx), " | transformational:",
    ncol(matKeep) - 1L - length(neoIdx), "\n")

# Blind: no ecology argument — same phyDat, same neomorphic classification.
mkd <- MkPrimeData(pdForDetect, neomorphic = neoIdx)
cat("MkPrimeData built:",
    " nTip=", mkd$nTip, " nChar=", mkd$nChar, "\n", sep = "")

# ---------------------------------------------------------------------------
# Resume from patched checkpoint.
# ResumeMkPrime uses mcmc (incl. nIter) from the checkpoint; model from ckp.
# ---------------------------------------------------------------------------
cat("Resuming from checkpoint...\n")
t0  <- Sys.time()
res <- ResumeMkPrime(checkpointFile = ckpFile, data = mkd)
cat("Elapsed:", format(Sys.time() - t0), "\n")

# ---------------------------------------------------------------------------
# Save combined result (pilot + continuation samples).
# ---------------------------------------------------------------------------
discard <- function(idx) idx[seq.int(ceiling(length(idx) / 4) + 1L,
                                      length(idx))]
samples <- ReadMkLog(logFile)
samples <- samples[discard(seq_len(nrow(samples))), , drop = FALSE]

modelBlind <- MkPrimeModel(
  ecologyAware = FALSE,
  kPrimePrior  = "geometric",
  coding       = "variable",
  expSteps     = 10
)

saveRDS(
  list(mkd     = mkd,
       model   = modelBlind,
       mcmc    = ckp$mcmc,
       res     = res,
       samples = samples),
  file.path(outRoot, "results", "rodent-blind-v2-result.rds")
)
cat("Saved result to", file.path(outRoot, "results",
                                  "rodent-blind-v2-result.rds"), "\n")
cat("\nDone.\n")
