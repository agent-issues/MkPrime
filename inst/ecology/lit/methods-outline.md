...
- Newick construction via `.BuildConvergentTree()` in
  `inst/ecology/simulations/sim3-helpers.R`.
- Branch lengths (v3 config, set in `inst/ecology/hamilton/sim3-multirep-v3/run_rep.R`):
...
- Simulation function: `.SimulateMkPrimeEcology()` in
  `inst/ecology/simulations/sim3-simulate.R` (normalize = TRUE, baseRate = 1.0,
...
- Parameters: nNeo = 100, nTrans = 200, φ = 4, π₀ = 0.75, θ = 1.0
  (for v4a/b/c); see `inst/ecology/simulations/sim3v4-params.R`.
...
- **v4-cross geometry**: ecology-1 assigned to `{A1..A4, B1..B4}` — the same
  ...
  [Helper: `inst/ecology/simulations/sim3v4cross-helpers.R`]
...
- First 25 % of post-warmup retained samples discarded: `keepFrom = ceiling(n/4) + 1`
  (function `.DiscardBurnin()` in `inst/ecology/simulations/sim3-scoring.R`).
...
- Unique-split comparison: splits unique to each consensus identified via
  ...
  `inst/ecology/scripts/rodent-comparison/rodent-comparison.R`).
...
- All simulation and analysis code is in the MkPrime package repository under
  `inst/ecology/hamilton/` (HPC dispatch scripts) and `inst/ecology/simulations/`
