...
`inst/ecology/simulations/sim3-scoring.R` replaces `hasBipart` with
...
- `inst/ecology/simulations/sim3-scoring.R` — root-invariant helpers.
- `inst/ecology/scripts/rescore-sim3.R` — re-scoring driver (Hamilton-ready).
- `inst/ecology/scripts/rescore-sim3-results.{rds,csv}` — tidy scores
  (corrected and legacy side by side) for all 22 chains.
- `inst/ecology/scripts/rescore-sim3-root-dist.csv` — per-chain root-position
  distributions (3–20 distinct rootings observed per chain).
- `inst/ecology/scripts/rescore-sim3-report.md` — this report.
...
`source("inst/ecology/simulations/sim3-scoring.R")` and call
