# run_rodent_blind_v2_cont2.R --------------------------------------------------
# Second continuation of the rodent BLIND Mk' MCMC v2 (1M iter reached,
# minESS=38, 359 trees).  Extends to 5M total iterations (~4M additional)
# to reach minESS >= 200.
#
# Strategy: patch checkpoint$mcmc$nIter to 5000000L so ResumeMkPrime runs
# the remaining ~4M iterations beyond where cont1 stopped.
# thin and treeThin are left unchanged — the chain accumulates more samples
# naturally as the iteration count grows.
#
# Checkpoint: /nobackup/pjjg18/mkp-rodent-blind-v2/rodent-blind-v2.ckp
#
# Usage (via SBATCH script):
#   Rscript run_rodent_blind_v2_cont2.R <nexFile>
#
...
if (!file.exists(ckpFile)) {
  stop("Checkpoint not found: ", ckpFile,
       "\nRun run_rodent_blind_v2.R (and cont) first.")
}

nIterTotal <- 5000000L  # 1M done + 4M continuation = 5M total

cat("=== Rodent BLIND v2 CONTINUATION 2 ===\n")
cat("nexFile:   ", nexFile, "\n")
cat("ckpFile:   ", ckpFile, "\n")
...
# ---------------------------------------------------------------------------
# Patch checkpoint: extend nIter to 5M so ResumeMkPrime runs the remainder.
...
# ---------------------------------------------------------------------------
# Rebuild MkPrimeData — identical to run_rodent_blind_v2.R so character
...
# ResumeMkPrime uses mcmc (incl. nIter = 5M) from the checkpoint.
# ---------------------------------------------------------------------------
