# 9-way CID comparison, adding mk_k40 to the 8-way arms.
# Tests whether the flexibility ramp continues past k=24 or plateaus.
...
saveRDS(df, file.path(OUT_DIR, "cid_nine_prior.rds"))
...
cat("\n=== mk_k24 vs mk_k40 (paired) ===\n")
diff_fk <- wide_fk$mk_k40 - wide_fk$mk_k24
cat(sprintf("  mean(k40 - k24)   = %+.5f\n", mean(diff_fk, na.rm = TRUE)))
cat(sprintf("  median(k40 - k24) = %+.5f\n", median(diff_fk, na.rm = TRUE)))
cat(sprintf("  k40 wins (k40 < k24): %d / %d\n",
            sum(diff_fk < 0, na.rm = TRUE), sum(!is.na(diff_fk))))
cat(sprintf("  k40 loses:            %d\n", sum(diff_fk > 0, na.rm = TRUE)))
cat(sprintf("  ties:                 %d\n", sum(diff_fk == 0, na.rm = TRUE)))
...
# ---- Monotonicity ramp summary (kp1..k40) -----------------------------------
ramp_arms <- c("mk_kp1", "mk_kp2", "mk_k9", "mk_k15", "mk_k24", "mk_k40")
...
cat(sprintf("  k24->k40  = %+.4f\n", ramp_means["mk_k40"] - ramp_means["mk_k24"]))
...
# ---- Headline plot ----------------------------------------------------------
png(file.path(OUT_DIR, "cid_nine_prior.png"),
    width = 2200, height = 700, res = 110)
...
mtext(side = 1, line = 3.5,
      text = sprintf("n_tasks: %s",
                     paste(paste0(ARMS, "=", table(df$arm)), collapse = ", ")),
      cex = 0.6, col = "grey40")

# k24 vs k40 paired-diff histogram
hist(diff_fk, breaks = 30,
     col = "#003399", border = "grey30",
     xlab = "CID(mk_k40) - CID(mk_k24)",
     main = sprintf("k40 vs k24 (n=%d common tasks)", length(fixedK_common)))
abline(v = 0, lty = 2, col = "grey40")
abline(v = mean(diff_fk, na.rm = TRUE), col = "red", lwd = 2)

# Ramp plot (kp1..k40 x-axis)
ramp_k <- c(mk_kp1 = 1, mk_kp2 = 2, mk_k9 = 9, mk_k15 = 15,
            mk_k24 = 24, mk_k40 = 40)
...
par(op); dev.off()
cat("\nSaved: cid_nine_prior.png and cid_nine_prior.rds\n")
