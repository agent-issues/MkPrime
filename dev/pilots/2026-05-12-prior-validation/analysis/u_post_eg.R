# Corrected u_post audit: per-character (E[k'|data] - kObs) from CORRECT-aligned
# mkp_eg summaries. Run after the 2026-05-17 alignment bug fix.

SUMMARY_DIR <- "dev/pilots/2026-05-12-prior-validation/summary"
OUT_DIR     <- "dev/pilots/2026-05-12-prior-validation/analysis"

files <- list.files(SUMMARY_DIR, "^mkp_eg_t[0-9]+_r[0-9]+\\.rds$",
                    full.names = TRUE)
cat("Found", length(files), "mkp_eg summary files\n")
...
saveRDS(df, file.path(OUT_DIR, "u_post_eg.rds"))

png(file.path(OUT_DIR, "u_post_eg.png"),
    width = 1200, height = 500, res = 110)
op <- par(mfrow = c(1, 2), mar = c(4, 4, 2.5, 1))
hist(df$u_post, breaks = 50, col = "darkgreen", border = "white",
     xlab = "u_post = E[k'|data] - kObs",
     main = paste0("mkp_eg per-char u_post (n=", nrow(df), ")"))
...
boxplot(u_post ~ kobs, data = df, xlab = "kObs", ylab = "u_post",
        main = "u_post by kObs", col = "lightgreen")
