...
test_that("MkPrimeModel kPrimePrior defaults to geometric", {
  model <- MkPrimeModel()
  expect_equal(model$kPrimePrior, "geometric")
})
...
test_that("print.MkPrimeModel runs without error for geometric model", {
  model <- MkPrimeModel()
  expect_invisible(print(model))
  expect_equal(model$kPrimePrior, "geometric")
})
